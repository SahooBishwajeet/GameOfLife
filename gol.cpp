// TO-DO :: More Cache-Friendly Code
...
bool isAlive(std::array<std::array<int, SCREEN_HEIGHT>, SCREEN_WIDTH> arr,
             int a, int b) {
  ...
  // POINT'S FATE For The Next Generation (Alive OR Dead) <Alive :: True, Dead
  // :: False>
...
int main() {
  ...
  while (true) {
    ...
    // Do All The Necessary Operatios To Render, Plot/Draw & Clear Screen (For
    // Next Generation) Note That All Are Wrapper Function For SDL2 Library
    // Functions
    screen.update();
    SDL_Delay(25);
    screen.input();
